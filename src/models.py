...
class GradualExpansionUNet(nn.Module):
    def __init__(self, in_msi=12, in_hsi=230, interpolation_mode='ConvTranspose2d'):
        """
        ...
        # [B, 12, H, W]
        self.inc = DoubleConv(in_msi, 64) # [B, 64, H, W]

        self.down1 = nn.Sequential(nn.MaxPool2d(2), DoubleConv(64, 128))  # [B, 128, H/2, W/2]
        self.down2 = nn.Sequential(nn.MaxPool2d(2), DoubleConv(128, 256)) # [B, 256, H/4, W/4]
        self.down3 = nn.Sequential(nn.MaxPool2d(2), DoubleConv(256, 512)) # [B, 512, H/8, W/8]
        ...
        self.up1b = DoubleConv(512, 384)    # [B, 384, H/4, W/4]
        # Concat with down1: [B, 256+128, H/2, W/2]
        self.up2b = DoubleConv(384, 256)    # [B, 256, H/2, W/2]
        # Concat with inc: [B, 192+64, H, W]
        self.up3b = DoubleConv(256, 230)    # [B, 230, H, W]
...
class UNet(nn.Module):
    def __init__(self, in_channels=230, base_features=64, interpolation_mode='ConvTranspose2d', learning_mode='Standard'):
        """
           interpolation_mode in ['ConvTranspose2d', 'Bilinear']. With ConvTranspose2d, 
           the upsampling is learnable (and could therefore introduce artefacts). With Bilinear, 
           the upsampling is smoother.
        ...
        self.inc = DoubleConv(in_channels, base_features)           # [B, 64, H, W]
        self.down1 = nn.Sequential(nn.MaxPool2d(2), DoubleConv(base_features, base_features*2))   # [B, 128, H/2, W/2]
        self.down2 = nn.Sequential(nn.MaxPool2d(2), DoubleConv(base_features*2, base_features*4)) # [B, 256, H/4, W/4]
        self.down3 = nn.Sequential(nn.MaxPool2d(2), DoubleConv(base_features*4, base_features*8)) # [B, 512, H/8, W/8]
        ...
        self.up1b = DoubleConv(base_features*8, base_features*4)    
        self.up2b = DoubleConv(base_features*4, base_features*2)    
        self.up3b = DoubleConv(base_features*2, base_features)    
